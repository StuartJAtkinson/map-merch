...
- [ ] **Cloud SQL `hoas-db` public IP exposed** — instance has `ipv4Enabled=true` with **no authorized networks** and `sslMode: ALLOW_UNENCRYPTED_AND_ENCRYPTED` (`requireSsl=false`). Backend connects via the Cloud SQL socket, so public IP isn't needed — disable public IP (or at least set `requireSsl`/`sslMode=ENCRYPTED_ONLY`). *(found 2026-06-04)*
- [ ] **No app-level password reset / account recovery** — auth has register/login/refresh/me but no way to reset a forgotten password; recovery requires direct DB surgery (see resolved item below). Add a reset flow. *(found 2026-06-04)*
